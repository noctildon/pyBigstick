...
!
!  MODIFIED in 7.9.12 (March 2021) to handle only 1 MPI process, in order to expedite better handling
!  of greedy opbundles
!
! INPUT:
...
subroutine intron_master_proc(fillup,printout,iprocs)
   use flags3body
   use jumplimits
   use nodeinfo
   use menu_choices
...
!   integer :: procstart,procstop
   integer iprocs  ! which MPI process to targer
   integer aerr,ierr
...
end subroutine intron_master_proc
!========================================================================= 
!
! original version, loops over all MPI processes
!
...
! print*,iproc,ibundle,opbundle(ibundle)%pxstart,startjump
          ...
          case ('NN')
             ...
             opbundle(ibundle)%nxstart = startjump
             opbundle(ibundle)%nxend   =stopjump
!........		     print*,iproc,' Updating opbundles...',ibundle,startjump,stopjump
...
!
		...
		if(containsexons)then  !check if another exon exists
			if(Xendcut(nex)<jumpend .and. Xjumpcut(nex+1) <=jumpend)then
				...
				print*,' To run, set compactjumpstorage_enabled=.false. in bmodules_parallel.f90 '
				print*,' STOPPING RUN '
				call BMPI_Abort(icomm,101,ierr)
				stop
