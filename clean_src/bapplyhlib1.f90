...
!......... USE CENTTROIDS..........
     if(applycentroids)then
	     call applycentroid(opbundlestart(iprocs),opbundleend(iprocs))
...
!$omp end parallel do
    ...
    if(kpow > 1 .and. Etarget /=0.)call subtractconstantE
...
!........... NOW REDUCE....................
   call clocker('wai','sta')    ! TIME HOW LONG WE ARE WAITING
   call BMPI_Barrier(fcomm2,ierr)  ! REMOVE WHEN DONE
...
!  NOTE: order of loops reversed in 7.9.10 to try to improve data locality
...
!		 if(iproc==18)print*,' Moar Testing ',xjmpstart,xjmpend
...
!-------- OpenMP functions ---------------------------------
  ...
  logical :: tryotherloops=.false.  ! deprecate
...
!print*,ibundle,numnthreads, 'threads 794 '
...
if(.not.tryotherloops)then
!     if(numnthreads > 1 .or. disableNoOMPloopswitch)then   !NOTE SHOULD BE DEFAULT
		 !  ALWAYS SET disableNoOMPloopswitch=.true.
...
!----------- FIND MATRIX ELEMENT -------------------------------------------
     ...
     else    ! DO NOT USE THIS LOOP; the problem is sorting both forwards and backwards on the same process
		 ! in principle I could use this if the 'f' and 'b' were on separate processes, but not always guaranteed
...
!... ADDED IN 7.9.10 
!
!  tor taking (H-Etarget)^k
!  this subtract Etarget*vec1 from vec2
!
subroutine subtractconstantE
	use localvectors
	use lanczos_info
	use nodeinfo
	implicit none
	...
	do ibasis = v1s,v1e
     	   vec2(ibasis) = vec2(ibasis)-Etarget*vec1(ibasis)
    end do
	
	return
	
end subroutine subtractconstantE
