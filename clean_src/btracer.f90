...
subroutine tracemaster
   ...
   integer :: aerr
!   getvariance = .true.   ! change this if you want 2nd moment/variance

   if(getvariance)then   ! COMPUTE BOTH FIRST AND SECOND MOMENTS
...
call BMPI_REDUCE(tr1,1,MPI_SUM,0,icomm,aerr)
...
203  format(' for dimension ',i8,', centroid = ',f12.6)
if(getvariance)then
     write(6,202)dimbasis,tr1,dsqrt(tr2)
 else
	 write(6,203)dimbasis,tr1
 end if
     write(6,*)' (saved to file trace.bigstick )'
!	 if(.not.getvariance)then
!		 write(6,*)' (to get variance, set getvariance=.true. in btracer.f90)'
     if ( writeout ) then
		 if(getvariance)then
		    write(resultfile,202)dimbasis,tr1,dsqrt(tr2)
	     else
		    write(resultfile,203)dimbasis,tr1
	     end if
	 end if
  ...
  write(89,*)' 2xM    Z    N    basis           centroid       width^2 '
    ...
    write(89,'(3i5,i8,2f20.6)')jz,np(1),np(2),dimbasis,tr1
    write(89,*)' 2xM    Z    N    basis           centroid       '	
end if
  close(89)
   return
...
!............SPE......................................
     if(call_spe)call applySPEbundled_trace(opbundlestart(iprocs), opbundleend(iprocs))
...
!........... PN....................................
		...
		if(applycentroids)call applyCENbundled_trace (opbundlestart(iprocs), opbundleend(iprocs))
...
!  subroutine applyspes
!
!  applies centroids-- purely diagonal
!
!====================================================================
subroutine applyCENbundled_trace (startbundle,endbundle )
  use tracy
...
!-------- OpenMP functions ---------------------------------
  ...
  integer(kind=basis_prec)   :: ip,in,psd,nsd,pstate,nstate,state
 ...
 !                  ibasis = ibasis+1
                ...
                metrace(state)%ncol = metrace(state)%ncol + 1
                if(.not.tracecount)then
                   metrace(state)%v( metrace(state)%ncol ) = vmx
                   metrace(state)%col( metrace(state)%ncol ) = state
                end if
 !               ibasis = ibasis+1				
  ...
  return
end subroutine applyCENbundled_trace
