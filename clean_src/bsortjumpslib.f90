...
!				   if(opbundle(ibundle)%hchar/='b')then
				   ...
				   call sort1bjumps(npjmps,p1b_isd(startp+1),p1b_fsd(startp+1),              &
...
! 
                ...
                call sort1bjumps(nnjmps,n1b_fsd(startn+1),n1b_isd(startn+1),              &
...
!                startn = opbundle(ibundle)%nsortstart-1
 !               nnjmps = opbundle(ibundle)%nsortend -startn 
                startn = opbundle(ibundle)%nsortstart-1
                nnjmps = opbundle(ibundle)%nsortend -startn 
! 				print*, ' Sorting on NN ',iproc,dosortjumps,startn,nnjmps,size(n2b_fsd)
...
! NOTE BY CWJ: However, do I need to sort these every time?
! 
!print*,iproc,' SORRRTINGGGG ',startn,nnjmps,ibundle
                
                if(dosortjumps)call sort2bjumps1op(nnjmps,n2b_fsd(startn+1),n2b_isd(startn+1),              &
                       n2b_op(startn+1), n2b_phase(startn+1))   

!............................................................................. PP; added in 7.6.1
			case('PP')  ! I'm turning this off with a dummy optype, doesn't speed up
				startp = opbundle(ibundle)%pxstart-1
			    npjmps = opbundle(ibundle)%pxend -startp 
!--------------------   Sort proton jump states on final SD, p1b_fsd
!--------------------   Order them to enable OpenMP for the forward application of H_pn
			    if(dosortjumps)then
!					if(opbundle(ibundle)%hchar=='b')then
					   call sort2bjumps1op(npjmps,p2b_fsd(startp+1),p2b_isd(startp+1),              &
					                          p2b_op(startp+1),  p2b_phase(startp+1))
!					else
 !					   call sort2bjumps1op(npjmps,p2b_isd(startp+1),p2b_fsd(startp+1),              &
! 					                          p2b_op(startp+1),  p2b_phase(startp+1))										  
...
subroutine sort2bjumps(ndim, x2b_isd, x2b_fsd, x2b_cop,x2b_dop,x2b_phase)
   use bmpi_mod
   use precisions
   implicit none
!---   Data passed in   
   ...
   integer(kind=4) :: x2b_cop(ndim)
   integer(kind=4) :: x2b_dop(ndim)
...
!        a=x1b_fsd(j)
         ...
         do 11 i=j-1,1,-1
            ...
            x2b_cop(i+1)=x2b_cop(i)
            x2b_dop(i+1)=x2b_dop(i)
...
12    continue
      ...
      call switch2b(ndim,k,l+1,x2b_isd,x2b_fsd,x2b_cop,x2b_dop,x2b_phase)

      if(x2b_fsd(l+1).gt.x2b_fsd(ir))then
         call switch2b(ndim,ir,l+1,x2b_isd,x2b_fsd,x2b_cop,x2b_dop,x2b_phase)
      end if
      if(x2b_fsd(l).gt.x2b_fsd(ir))then
         call switch2b(ndim,ir,l,x2b_isd,x2b_fsd,x2b_cop,x2b_dop,x2b_phase)
      end if
      if(x2b_fsd(l+1).gt.x2b_fsd(l))then
         call switch2b(ndim,l,l+1,x2b_isd,x2b_fsd,x2b_cop,x2b_dop,x2b_phase)
      end if
      i=l+1
      j=ir
      isd = x2b_isd(l)
...
!        if(x1b_fsd(j).gt.a)goto 4
      if(j.lt.i)goto 5
      call switch2b(ndim,j,i,x2b_isd,x2b_fsd,x2b_cop,x2b_dop,x2b_phase)
      goto 3
