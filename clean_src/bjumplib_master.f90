...
!=====================================================================
!
!  BJUMPLIB_MASTER.f90: master jump generation routines 
...
!==========================================================================
!
!  NOTE ADDED in 7.9.0.  In order to cap size of storage of jumps associated with opbundles
!  (which are denoted by sectorjumps), need to keep track of the number of jumps 
!  as one is looping over geneologies; the weld algorithm only breaks easily on geneologies
!
!
!==============================================================
module jump_mod
...
subroutine hoopmaster(hermflag,whermflagp,whermflagn,whermflag2p, whermflag2n ,change1bodyqs,change2bodyqs,change3bodyqs)
...
!----------- FLAGS FOR RESTRICTION ON CHANGING q#s in jumps-----------------
  ...
  logical    :: whermflag2p, whermflag2n  ! for 2-body
  integer :: ith, its
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
!.....  PAY ATTENTION TO HERMITICITY!!!
		...
		call split_sectorjumps(2,1)
		
        call master2bodyjumps(2,.false.)
		call split_sectorjumps(2,2)
...
! --- END OF 3-BODY OPERATION
     ...
     if(np(1) > 1)then	 
         ...
         call set2bsectorjumps(1,.false.,  hermflag, whermflag2p ,change2bodyqs, .true., 2 )
         call set2bsectorjumps(1,.true.,  hermflag,whermflag2p  ,change2bodyqs,.true., 2 )
         call masterfindconjumps2b(1,.false.)     ! NOTE: Need to find conjugate jumps because if none, then skip
         call master2bodyjumps(1,.false.  )
		 call split_sectorjumps(2,1)
		 call masterfindconjumps2b(1,.true.) 

     endif

     if(np(2) > 1)then
         call set2bsectorjumps(2,.false.,  hermflag, whermflag2n ,change2bodyqs,.true., 2 )
         call set2bsectorjumps(2,.true.,  hermflag, whermflag2n ,change2bodyqs,.true., 2 )
         call masterfindconjumps2b(2,.false.)
         call master2bodyjumps(2,.false. )
	     call split_sectorjumps(2,2)
		 call masterfindconjumps2b(2,.true.) 
...
! NOTE that I use "hermiticity" on 1-body jumps but not "W-hermiticity"
        ...
        call masterfindconjumps1b(1,change1bodyqs,.false.)
        call masterfindconjumps1b(2,change1bodyqs,.false.)  ! not really needed
...
!--------------- find conjugate jumps AGAIN
        call masterfindconjumps1b(1,change1bodyqs,.true.)

        call masterfindconjumps1b(2,change1bodyqs,.true.)  ! not really needed
...
!----------- FLAGS FOR RESTRICTION ON CHANGING q#s in jumps-----------------
   ...
   if(.not. ham_readin .and. menu_char /= '2b')then
	   if(iproc==0)print*,' Not creating jumps ! '
	   return
   end if
...
!..................... PPP/NNN............................
 ...
 !       if(ham_readin)call master3bodyjumps(2,.true., change3bodyqs)
        call master3bodyjumps(1,.true., change3bodyqs)
        call master3bodyjumps(2,.true., change3bodyqs)
!.......................PPN/PNN ........................
!........... set up sector jumps ................
!......... NB: I do not use hermiticity for 1-body jumps, but automatically do for 2-body jump
...
!------- VARIABLES ADDED 7.9.0 FOR SPLITTING SECTORJUMPS
  integer(8),allocatable :: splitstart(:)
  integer(4) :: nsplit
  integer(8) :: laststart,jumpcap
...
!			print*,iproc,' ALLOCAMATING ',minjump,maxjump			
...
!		   print*,iproc,' Allocaty ',restrictjumps, startnnjumps(iproc),minjump
...
!   MODIFIED IN 7.9.1 to try to get all possible orderings
     call matchmaker(it,nbody,is,descent%sector(is),nbody,fs,descent%sector(fs),.not.dens2bflag, ngeneologies)

     if ( ngeneologies == 0 ) cycle
	 
     njumps =   x2bjump(it)%sjmp(isjmp)%nstart
     curgen =>geneology
!........ ADDED 7.9.0: When looping over geneologies, check on jumps accumulated.
!         If they exceed the cap on jumps, set up for a "split" of jumps	 

     if(.not.fill)then
		 nsplit = 0
		 allocate(splitstart(0:ngeneologies))   ! won't need this many, but safe upper limit
...
!        call weld(it,is,fs,nbody,isjmp,.true., fill , x2bjump(it)%sjmp(isjmp),startjump,endjump,shiftjump,njumps)
		...
		if(.not.fill)then
			...
			if(njumps-splitstart(nsplit) > jumpcap)then   ! BACK UP TO LAST GENEOLOGY
				if(igen==1)cycle  ! NEED TO INCLUDE AT LEAST ONE GENEOLOGY
				
				nsplit = nsplit+1
				splitstart(nsplit)=laststart
...
!......... ADDED 7.9.0: put split information into sector jump for later modification..........	 
...
!............. FILL IN
              x2bjump(it)%sjmp(isjmp)%nsplit=nsplit
			  if(nsplit> 0)	then 
				  
				  allocate (x2bjump(it)%sjmp(isjmp)%splitstart(0:nsplit))
				  x2bjump(it)%sjmp(isjmp)%splitstart = splitstart(0:nsplit)
				  
			  end if
			 
			 deallocate(splitstart) 
     end if
...
!............. ADDED IN 7.9.0............
!     FIRST, BROADCAST ALL THE SPLITS IN SECTOR JUMPS
	 
     do isjmp = 1,x2bjump(it)%nsectjumps
         jumpcount(isjmp) = x2bjump(it)%sjmp(isjmp)%nsplit
     end do
     call BMPI_ALLREDUCE(jumpcount,jumpreduce,x2bjump(it)%nsectjumps, & 
          MPI_SUM,icomm,ierr)
     do isjmp = 1,x2bjump(it)%nsectjumps
         x2bjump(it)%sjmp(isjmp)%nsplit = jumpreduce(isjmp)  
     end do 
	 
!....... CHECK IF SPLITS; IF SO, SEND AROUND.......
...
!
!  NB:   WHEN CREATING JUMPS FOR 1-BODY DENSITY MATRICES, HERMITICITY TURNED OFF;
...
!  This has consequences for computing densities
!
!  NB:   WHEN CREATING JUMPS FOR 2-BODY DENSITY MATRICES, HERMITICITY TURNED OFF;
!  THIS MEANS NO ORDERING ON INITIAL, FINAL SECTORS
!
! CALLS
!  maxdelJz
...
subroutine set2bsectorjumps(it,create,hermflag,whermflag,changeq,changew,ndm )
   ...
   if(.not.create)then
	  ...
	  x2bjump(it)%sjmp(:)%nsplit=0    ! added in 7.9.0
   endif
...
! CALLS
!    findconjugatejumps  
!    findconjugatesemidiag
!
   subroutine masterfindconjumps1b(it,changeq,fill)

   use jumpNbody

   implicit none
   integer it
   logical changeq,fill
   
   if(changeq)then
      call findconjugatejumps(it,.false.,x1bjump,fill)
      if(fill)call findconjugatejumps(it,.true.,x1bjump,fill)
   else
      call findconjugatesemidiag(it,.false.,x1bjump,fill)
      if(fill)call findconjugatesemidiag(it,.true.,x1bjump,fill)
   endif
   return
...
!
! CALLED BY hoopmaster
!
! CALLS  findconjugatesemidiag
!
   subroutine masterfindconjumps2b(it,fill)
   ...
   call findconjugatesemidiag(it,.false.,x2bjump,fill)
   if(fill)call findconjugatesemidiag(it,.true.,x2bjump,fill)
...
!  masterfindconjumps1b
!
   subroutine findconjugatejumps(it,create,xNjumps,setup)
...
!------------- ERROR TRAP VARIABLES --------------
   ...
   if(.not.create) then
	   if(associated(xNjumps(it)%csjmp))nullify(xNjumps(it)%csjmp)
...
!.................... TESTING.......................................
     ...
     if(.not.create)then
		 ...
		 if(setup)then
		 if(associated(xNjumps(it)%csjmp(isjmp)%cjump))nullify(xNjumps(it)%csjmp(isjmp)%cjump)
...
!  (e.g., pp 2-body jumps with n 1-body jumps for a ppn 3-body force)
!  
!  CALLED BY: masterXXYconjugatejumps
!
...
!  masterfindconjumps2b
!
   subroutine findconjugatesemidiag(it,create,xNjumps,setup)
...
!
   ...
   call findconjugatesemidiag(it,.false.,x3bjump,.true.)
   call findconjugatesemidiag(it,.true.,x3bjump,.true.)
...
!============================================================================
!
! ADDED in 7.9.0 (Jan 2019) to try to limit storage on jumps
! by splitting up sector jumps
!
! 3-BODY NOT YET IMPLEMENTED
!

subroutine split_sectorjumps(Nbody,it)
	use jumpdef
	use jumpNbody
	use flagger
	use nodeinfo
	...
	use jump3body
	implicit none
	integer,intent(in) :: Nbody  ! whether 1-, 2-, or 3-body jumps
	integer,intent(in) :: it    ! species
	
    type (jumpsect), pointer :: xNbjump
	type(jumpsect) :: xtmpjump
		
	integer :: nsectjumps0,nsectjumpsX
	integer, allocatable :: split(:)	
	integer :: isj,jsj,k
	integer(8) :: maxjumpsallowed,localjumps,maxjumpsfound
	integer(8) :: stride,nstart
	logical :: skipme = .false.  ! for testing purposes only
	
	select case (Nbody)
	   case (1)
	      xNbjump => x1bjump(it)
		  maxjumpsallowed = (maxjumpmemory_default*maxfrac_sectjump_store * 1e9)/bytesper1Bjump
	   ...
	   case (3)
          xNbjump => x3bjump(it)
	      maxjumpsallowed = (maxjumpmemory_default*maxfrac_sectjump_store * 1e9)/bytesper3Bjump
		  
	   case default   
	   print*,' Error in split_sectorjumps  in bjumplib_master.f90'
	
    end select
	
	nsectjumps0 = xNbjump%nsectjumps
	
	if(nsectjumps0==0)return
	
	allocate(split(nsectjumps0))
	allocate(xtmpjump%sjmp(nsectjumps0),xtmpjump%isector(nsectjumps0),xtmpjump%fsector(nsectjumps0))
	split=0
	xtmpjump%nsectjumps = nsectjumps0
	nsectjumpsX = nsectjumps0
	maxjumpsfound = 0
	do isj = 1,xNbjump%nsectjumps
      
		localjumps = xNbjump%sjmp(isj)%njumps
		maxjumpsfound = max(maxjumpsfound,localjumps)
		if(localjumps > maxjumpsallowed)then
		    split(isj)= localjumps/maxjumpsallowed	
			nsectjumpsX = nsectjumpsX+split(isj)
		end if
		xtmpjump%isector(isj)= xNbjump%isector(isj)
		xtmpjump%fsector(isj)= xNbjump%fsector(isj)
		xtmpjump%sjmp(isj)%njumps= xNbjump%sjmp(isj)%njumps
		xtmpjump%sjmp(isj)%nstart= xNbjump%sjmp(isj)%nstart
		xtmpjump%sjmp(isj)%diag= xNbjump%sjmp(isj)%diag
    ...
    if(iproc==0)then
		...
		case (1)
!		print*,it,' max # ',Nbody,'-body jumps (in a sector jump) ',maxjumpsfound
		write(logfile,'(2x,i2," max # 1-body jumps in a sector jump ",i15," => ",f9.3," Gb storage" )')it,maxjumpsfound, & 
		maxjumpsfound*bytesper1bjump*1.0e-9
		
		case(2)
		write(logfile,'(2x,i2," max # 2-body jumps in a sector jump ",i15," => ",f9.3," Gb storage" )')it,maxjumpsfound, & 
		maxjumpsfound*bytesper2bjump*1.0e-9
		
		case(3)
		write(logfile,'(2x,i2," max # 3-body jumps in a sector jump ",i15," => ",f9.3," Gb storage" )')it,maxjumpsfound, & 
		maxjumpsfound*bytesper3bjump*1.0e-9
		
	    end select
		
		
	end if
	if(nsectjumps0 == nsectjumpsX .or. skipme)then
		deallocate (split,xtmpjump%sjmp, xtmpjump%isector,xtmpjump%fsector)
		return ! no splitting
	end if
	
	if(iproc==0  )then
		write(6,*)Nbody,it,' added ',nsectjumpsX -nsectjumps0,'sector jumps from splitting '

		write(logfile,*)Nbody,it,' added ',nsectjumpsX -nsectjumps0,' sector jumps'
		write(logfile,*)' maxjumpmemory_default = ',maxjumpmemory_default,', maxfrac_sectjump_store = ', maxfrac_sectjump_store
	end if
	
	select case (Nbody)
	   case (1)
	   deallocate(x1bjump(it)%sjmp,x1bjump(it)%isector,x1bjump(it)%fsector)
   	   allocate(x1bjump(it)%sjmp(nsectjumpsX),x1bjump(it)%isector(nsectjumpsX),x1bjump(it)%fsector(nsectjumpsX))
	   xNbjump=> x1bjump(it)
	   
	   case(2)
	   deallocate(x2bjump(it)%sjmp,x2bjump(it)%isector,x2bjump(it)%fsector)
   	   allocate(x2bjump(it)%sjmp(nsectjumpsX),x2bjump(it)%isector(nsectjumpsX),x2bjump(it)%fsector(nsectjumpsX))
	   xNbjump=> x2bjump(it)
	   
	   case(3)
	   deallocate(x3bjump(it)%sjmp,x3bjump(it)%isector,x3bjump(it)%fsector)
   	   allocate(x3bjump(it)%sjmp(nsectjumpsX),x3bjump(it)%isector(nsectjumpsX),x3bjump(it)%fsector(nsectjumpsX))
	   xNbjump=> x3bjump(it)
    ...
    xNbjump%nsectjumps = nsectjumpsX
	if(nsectjumpsX /= nsectjumps0 .and. iproc == 0)then
		print*,' increase sector jumps from ',nsectjumps0,' to ',nsectjumpsX
	end if
	jsj = 0
	do isj = 1,nsectjumps0
		stride = xtmpjump%sjmp(isj)%njumps/(split(isj)+1)			
		nstart =  xtmpjump%sjmp(isj)%nstart
!		if(split(isj)>0 .and. k > 0 .and. iproc==0)print*,' adding sector jump to ',isj,' by ',split(isj)
		
		do k = 0,split(isj)
		   jsj = jsj+1
!		if(split(isj)>0 .and. k > 0 .and. iproc==0)print*,' adding sector jump ',jsj,split(isj)
		   xNbjump%isector(jsj)=xtmpjump%isector(isj)
		   xNbjump%fsector(jsj)=xtmpjump%fsector(isj)
		   xNbjump%sjmp(jsj)%diag =xtmpjump%sjmp(isj)%diag
		   xNbjump%sjmp(jsj)%nstart =nstart
		   if(k<split(isj))then
		      xNbjump%sjmp(jsj)%njumps= stride
              nstart = nstart+stride
		  else
		      xNbjump%sjmp(jsj)%njumps= xtmpjump%sjmp(isj)%njumps -k*stride
		  end if
...
! .... TESTING ONLY...
!		if(split(isj)>0)then
!			print*,' checking: ',xtmpjump%sjmp(isj)%nstart,xtmpjump%sjmp(isj)%nstart+xtmpjump%sjmp(isj)%njumps
!			do k = split(isj),0,-1
!				print*,xNbjump%sjmp(jsj-k)%nstart,xNbjump%sjmp(jsj-k)%nstart+xNbjump%sjmp(jsj-k)%njumps
!			end do
!		end if
...
end subroutine split_sectorjumps
