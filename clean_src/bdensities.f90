...
!		end if
        ...
        denmat(mya,myb,tt) = denmat(mya,myb,tt) + cleb(ja,ma,jb,-mb,2*Jt,0)*iphase & 
            * tsign* x1bopme(ia*asgn,ib*bsgn)*cleb(1,tsign,1,-tsign,2*Tt,0) /(cg*cgt)
           end do
         else
            denmat(mya,myb,it-1) = denmat(mya,myb,it-1)+ cleb(ja,ma,jb,-mb,2*Jt,0)*iphase & 
            *  x1bopme(ia*asgn,ib*bsgn)/cg
         endif
