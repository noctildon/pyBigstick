...
!---------------------------------------
  ...
  if( menu_char =='2b' )return   ! two-body dnensities--do not readin
...
!  if(subsume_spe)call subsume_sp_pot_into_2body
  if(.not.writejumpstart)call broadcastTBMES 
	  ...
	  if(.not.success)then  ! try to open a MFD file 	
	      ...
	      if(success)then 
	         ...
	         inquire(file=int_path(1:length_int_path)//intfilename(1:ilast),exist=success) 
	 	      if(success)then 
	            open(unit=1,file=int_path(1:length_int_path)//intfilename(1:ilast),status='old',err=222) 
				...
				222 continue
				print*,' could not find file, maybe error?'
				return
	 	      end if 
	      end if 		  
	  end if   ! not success
...
if(.not.autoscale)then
  if(auto_input)then 
     ...
     if(.not. coul_int)then 
        print*,' Enter scaling: spescale, A0,A,X '
        print*,'( spescale scales single particle energies, '
        print*,' while TBMEs are scaled by (A0/A)^X ) for TBMEs ' 
        print*,' (If A or X = 0, then TBMES scaled by A0 ) ' 
		print*,' See manual for examples. '
...
!     end do 
!  endif 
  if(pp_int .or. iso_int)pspe(1:numorb(1)) = pspe(1:numorb(1)) + spscale*spetmp(1:numorb(1)) 
  if(nn_int .or. iso_int)nspe(1:numorb(2)) = nspe(1:numorb(2)) + spscale*spetmp(1:numorb(2)) 
  ...
  if(autoscale)then
	  if(spetmp(numorb(1)+2)==0.0 .or. spetmp(numorb(1)+3)==0.0)then
		  vscale = 1
		  if(iproc==0)then
			  print*,' Autoscaling  = 1 '
			  write(logfile,*)' Autoscaling = 1 '
		  end if
	  else
...
!---------- ERROR TRAP ADDED July 2011 CWJ ------------------' 
     ...
     if(.not. mixed_T)then 
        read(1,*,err=1899,end=1999)ia,ib,ic,id,j,t,vv 

        if(i==1 .and. iproc==0)then
			print*," As a check, first two-body matrix element in list is ",vv 
			write(logfile,*)" As a check, first two-body matrix element in list is ",vv 
...
!---------- ERROR TRAP ADDED May 2020 CWJ ------------------' 
         if( (ia==ib .or. ic==id) .and.( mod(j+t,2)==0 .and. vv /= 0.0))then
			 if(iproc==0)then
				 write(6,*)' Illegal matrix element ',ia,ib,ic,id,', J T = ',j,t
				 write(6,*)' Should not exist '
			 end if
			 stop
		 end if
...
1899 continue
  if(iproc==0)then
    ...
    print*,nme,i
	if(i > 10 .and. i < nme)then
		print*,' OR '
		print*,' You may have computed the wrong number of two-body matrix elements '
		print*,' Found only ',i-1,' out of ',nme, ' expected'
		return
	end if
    backspace(1)
    read(1,'(a)')title
	...
	1999 continue
	  if(iproc==0)then
			...
			print*,' Your file may have the wrong number of two-body matrix elements '
			print*,' Found only ',i-1,' out of ',nme, ' expected'
			print*,' Will continue on, but be aware!!!  '
			write(logfile,*)' Your file may have the wrong number of two-body matrix elements '
			write(logfile,*)' Found only ',i-1,' out of ',nme, ' expected'
			write(logfile,*)' Will continue on, but be aware!!!  '
		end if
...
subroutine readv2bme_xpn(firstread,formattedfile,intcase) 
  ...
  logical autoscale  ! added 7.8.1 for reading in NuShell files
  integer :: numval
  logical :: dupliflag ! USED ONLY FOR UPN TO PREVENT DOUBLE COUNTING
!----------------------------------------------- 
  character*1 ychar 
...
! set to values that would cause problems if used 
  ...
  if(iproc==0)then
	  print*,' Assuming unnormalized p-n matrix elements '
	  print*,' IMPORTANT: MUST READ IN FIRST '
  end if
  normalizedpn = .false.
...
!-------------- ENTER SCALING ----------------------------- 
  ...
  if(auto_input)then 
	  ...
	  if(usescale)then
          print*,' Enter global scaling for spes, A0,A,X ( (A0/A)^X ) for TBMEs '
...
!-------------- READ IN SPEs --------------
  ...
  if(autoscale)then
	  ...
	  pspescale = 1
	  nspescale = 1
	  if(spetmp(numorb(1)+numorb(2)+2)==0.0 .or. spetmp(numorb(1)+numorb(2)+3)==0.0)then
		  vscale = 1
		  if(iproc==0)then
			  print*,' Autoscaling  = 1 '
			  write(logfile,*)' Autoscaling = 1 '
 	     ...
 	     do i = 1,numorb(1)
 	   	  pspe(i)=pspe(i) +spetmp(i)
 	     end do
  
 	     do i = 1,numorb(2)
 	   	  nspe(i)=nspe(i)+spetmp(i+neutron_offset)
 	     end do
...
!  endif 
  ...
  dupliflag = .false.
  if(.not.normalizedpn)then  !
	  write(6,*)' There are ',nme,' UPN-formatted matrix elements; I expect ',nv2bme_tot
	  if(nme > nv2bme_tot)dupliflag = .true.
  end if
...
!
  if(.not.normalizedpn )then  
	  print*,' Some matrix elements may be duplicated in this file by Hermiticity '
	  print*,' Therefore you should read this file FIRST before any others; '
	  print*,' Prior matrix elements will get overwritten '
  end if
...
!  OBLIGATORY ORDERING:  ia,ic proton, ib,id neutron
	 ...
	 if(a /=b .and. a ==d .and. b == c .and. dupliflag)factor=0.25  ! BUT THIS IS ONLY IF WE HAVE EXTRA MATRIX ELEMENTS
    end if	 
...
!	 if(normalizedpn)then
        pnme(indx)%v(j)=pnme(indx)%v(j)+vv*pnscale*phase*factor *vscale   ! ADD
!	 else
 !       pnme(indx)%v(j)=vv*pnscale*phase*factor *vscale  !OVERWRITE
!	 end if
...
!..................... FIND START, FINISH................ 
			...
			if(.not.dens2bflag)then
			    ...
			    XXcouples(it)%mestart(2) = neven*neven    ! ADDED IN 7.9.1 for two-body densities
			end if
...
! 
!  NOTE: what are called 'pairs' here
!  are elsewhere called 'couples'
!
...
subroutine set_tbme_XXarray(it) 
  ...
  integer cpairend,pairblocksize   ! ADDED 7.9.1
...
!.................... COMPUTE # OF MATRIX ELEMENTS 
  ...
  if(.not.dens2bflag)then
     ...
     nv = ncpair*ncpair         ! ADDED in 7.9.1
  end if
  ncpair = ncouplesXX(it) - XXcouples(it)%meref(2) 
  if(.not.dens2bflag)then
      nv = nv + ncpair*(ncpair+1)/2 
  else
	  nv = nv + ncpair*ncpair      ! ADDED in 7.9.1
  end if
  ...
  if(.not.dens2bflag)then
     if(it == 1)then 
      ...
      if(it == 1)then 
		 allocate( pp2bden( nv2bmedim(it) ), stat=aerr) 
         if(aerr /= 0) call memerror("set_tbme_XXarray 3") 
         xxden => pp2bden 
         cmap => PPcouplemap 
		 
      else 
         allocate( nn2bden( nv2bmedim(it) ), stat=aerr) 
         if(aerr /= 0) call memerror("set_tbme_XXarray 4") 
         xxden => nn2bden
         cmap => NNcouplemap 
      endif
  ...
  do dpair = 1,ncouplesXX(it) 
	 ...
	 if(dparity ==1)then
         pairblocksize = XXcouples(it)%meref(2)
	 else
         pairblocksize = ncouplesXX(it)-XXcouples(it)%meref(2)
     ...
     do cpair = 1, cpairend   
          ...
          if(.not.dens2bflag)then 
              indx = istart + (dpair-iref)*(dpair-iref-1)/2+cpair-iref 
			  
		  else
              indx = istart + (dpair-iref-1 )*pairblocksize+cpair-iref 			  
!			  print*,' INDEX ',cpair,dpair,indx
		  end if
		  ...
		  if(.not.dens2bflag)then
             ...
             xxme(indx)%jmax = jmax 
             if(jmax >= jmin) then 
                allocate( xxme(indx)%v(jmin:jmax), stat=aerr )  
                if(aerr /= 0) call memerror("set_tbme_XXarray 5") 
                xxme(indx)%v(:) = 0.0 	
			  
  			  if(a==b .or. c==d)then
  				  nv2bme_tot = nv2bme_tot + (jmax+1)/2
  			  else
  				  nv2bme_tot = nv2bme_tot + jmax - jmin+1
                end if	
             endif      
	      else
	          jabmin = abs(ja-jb)/2
	          ...
	          jcdmin = abs(jc-jd)/2
	          jcdmax =(jc+jd)/2
			  jmin = min(jabmin-jcdmax,jcdmin-jabmax)
			  jmin = max(0,jmin)
			  
			  jmax = jcdmax + jabmax
			  
!			  print*,' tEst',itbme,' jabmax ',jabmax,dpair,iref,pairblocksize,cpair,istart
              xxden(indx)%jabmin = jabmin 
              xxden(indx)%jabmax = jabmax 
              xxden(indx)%jcdmin = jcdmin 
              xxden(indx)%jcdmax = jcdmax 
              xxden(indx)%jmin = jmin 
              xxden(indx)%jmax = jmax 
			  
			  if(jabmin > jabmax)cycle
			  if(jcdmin > jcdmax)cycle
			  if(jmin > jmax)cycle
			  
			  
			  allocate(xxden(indx)%v(jabmin:jabmax,jcdmin:jcdmax,jmin:jmax),stat=aerr)
              if(aerr /= 0) call memerror("set_tbme_XXarray 6") 
              xxden(indx)%v(:,:,:) = 0.0 
...
!..................... FIND START, FINISH................ 
			...
			if(.not.dens2bflag)then
                ...
                PNcouples%mestart(2) = neven*neven   ! ADDED 7.9.1 for two-body densities
...
subroutine set_tbme_PNarray 
  ...
  integer cpairend, pairblocksize
  integer dparity 
  integer iref,istart 
  integer itbme 
  integer a,b,c,d 
...
!.................... COMPUTE # OF MATRIX ELEMENTS 
  ...
  if(.not.dens2bflag)then
	  ...
	  nv = ncpair*ncpair                       ! ADDED in 7.9.2 
  end if
  ...
  if(.not.dens2bflag)then
	  ...
	  nv = nv+ncpair*ncpair                       ! ADDED in 7.9.2 
  ...
  if(nv2bmedim(0) == 0)then 
     return 
	  ...
	  if(.not.dens2bflag)then
          ...
          allocate( pn2bden( nv2bmedim(0) ) , stat=aerr) 
          if(aerr /= 0) call memerror("set_tbme_PNarray 1b") 		  
     ...
     do cpair = 1, cpairend
		  ...
		  if(.not.dens2bflag)then
             itbme = istart + (dpair-iref)*(dpair-iref-1)/2+cpair-iref 
              ...
              itbme = istart + (dpair-iref-1)*pairblocksize+cpair-iref 
		  ...
		  if(.not.dens2bflag)then
             ...
             if( jmax >= jmin)then 
			  ...
			  jmin = min(jabmin-jcdmax,jcdmin-jabmax)
			  jmin = max(0,jmin)
              pn2bden(itbme)%jabmin = jabmin 
              pn2bden(itbme)%jabmax = jabmax 
              pn2bden(itbme)%jcdmin = jcdmin 
              pn2bden(itbme)%jcdmax = jcdmax 
              pn2bden(itbme)%jmin = jmin 
              pn2bden(itbme)%jmax = jmax 
              ...
              allocate( pn2bden(itbme)%v(jabmin:jabmax,jcdmin:jcdmax,jmin:jmax) , stat=aerr) 
              if(aerr /= 0) call memerror("set_tbme_PNarray 3") 
              pn2bden(itbme)%v(:,:,:) = 0.0 
		  end if
...
!  READ PAST ANY HEADER LINES	
   ...
   read(filenumber,*,err=111,end=111)n1me
   print*,n1me,' nonzero single particle scalar potential elements '
   
   if(n1me==0)return   

!........ ERROR TRAP.......
!   Bigstick currently does not handle off-diagonal potentials for exactly one particle
   if(n1me > 0  .and. abs(np(1)) + abs(np(2)) ==1)then
	   print*,'    '
	   print*,' Attention: you appear to have only one partile or one hole '
	   print*,' BIGSTICK currently does not handle off-diagonal one-body parts '
	   print*,' in that circumstance '
	   print*,'  '
	   stop
   end if
   ...
   do i = 1,n1me
	   ...
	   case ('xpn')
		...
		case('pns')   ! note this is not currently being used
