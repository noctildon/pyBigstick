...
!--------------- NEEDED FOR FINDING INDICES OF OPERATORS ----------
  ...
  integer(kind=8) :: cpair,dpair,cpair0,dpair0
...
!		 if(nbody==2)print*,' left  ',opleft(1:4),' right ',rcurch%chain%oplist(1:4)
...
!-------------ONE-BODY  --- a^+i a_j
          ...
          if(i < 0)then
           ...
           if(fill .and. njumps-xjump%nstart > xjump%njumps)then
               print*,' error, too many jumps (1): Nbody = ',nbody,' species ',it,' fill = ',fill
               print*,njumps,xjump%nstart,xjump%njumps
               stop
         endif
...
!------------------ FIND M, PAR OF OPERATORS
          ...
          if(first)then
           ...
           dpair = map(dpair)
           cpair = map(cpair)
		   myTRphase = 1                   ! ADDED 7.9.4
		   if(useTRphase .and. m < 0)then
			   myTRphase = XX2(it)%pair(cpair)%TRphase* XX2(it)%pair(dpair)%TRphase
			   
!			   print*,cpair,XX2(it)%pair(cpair)%TR,XX2(it)%pair(cpair)%m,m
			   cpair = XX2(it)%pair(cpair)%TR
			   dpair = XX2(it)%pair(dpair)%TR
		   ...
		   if(.not. dens2bflag)then   ! added in 7.9.1
              if(cpair <= dpair) then
...
!------------------ OPTION TO SKIP OVER ZERO MATRIX ELEMENT JUMPS ----------
           ...
           njumps = njumps + 1
           if(fill .and. njumps-xjump%nstart > xjump%njumps)then
               print*,iproc,' error, too many jumps (2): Nbody = ',nbody,' species ',it,' sector jump =',isj
               print*,njumps,xjump%nstart,xjump%njumps
			   call BMPI_Barrier(icomm,ierr)
			   call BMPI_Abort(icomm,333,ierr)
              stop
            endif
            if(fill .and. njumps >= minjump .and. njumps <=maxjump )then
...
!                    if(storeXXmesjumps)then   ! must decouple matrix element
 ...
 !                   end if
                 ...
                 phase(njumps-jumpshift)= int(lphase*rphase*cphase*dphase*genphase*myTRphase, kind=1)
            endif
