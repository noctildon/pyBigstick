...
!   setup_mpi_hist_comm:
!   br_set_histpos: sets variable "pos" = which vector
!
!   MOVING VECTORS AROUND
!   br_retrieve_hist(pos): takes vector pos from history and puts into working vector br_reg
!   br_add2hist(pos): takes working vector br_reg and pushes into br_hist as vector pos
!   br_grab_vec1:  takes vec1 and stores as working vector br_reg
!   br_grab_vec2:  takes vec2 and stores as br_reg
!   br_restore_vec1: pushes working vector br_reg to vec1
!   br_restore_vec2: pushes br_reg to vec2
!
!   LINEAR ALGEBRA ON VECTORS
!   function dot_product8   :  Takes dot product of two lanc_prec vectors with result
!                     in kind=8 precision; typically br_reg with itself or a vector in br_histbuf
!                     is only "local" contribution (from local slices) so must be reduced
...
module mod_reorthog
   ...
   integer :: maxkey   ! the above arrays are dimensioned 0:maxkey
...
subroutine setup_mpi_hist_comm()
   ...
   integer :: color, key, ierr,  pi, hsize
...
!   if(iproc==0)then
!	   do i = 0,nproc-1
!		   print*,i,br_ofraglist(i),nodal(i)%ifragment,nodal(i)%ffragment		   
!	   end do	   
!   end if
...
!   end if
   if(br_histpos > UBOUND(br_histbuf, 2)) then
      if(iproc==0) print *, "Trying to save too many Lanczos vectors.  Buffer depth=", UBOUND(br_histbuf, 2),br_histpos
      stop 23
   end if
...
!--------------------------------------- + -------------------------------------
!
!  TRANSFERS vec2 -> vec1 ON DIAGONAL ROOT 
!  then use MPI_BCAST
!
!  NOT CALLED (?)
!
...
!--------------------------------------- + -------------------------------------
!
!  TRANSFERS vec1 -> vec2 ON DIAGONAL ROOT 
!  then use MPI_BCAST
! 
!  CALLED BY:
!      lanczos_output
...
!$omp    shared(br_reg) 
   do istate = ostart, ostop
      br_reg(istate) = br_reg(istate) - real(a * br_histbuf(istate,br_histpos), kind=lanc_prec)
   end do
!$omp end parallel do
   return
...
! in history: vec(ivec) = vec(ivec) - vec(jvec) *( vec(ivec).vec(jvec))
!
!  CALLED BY: lanczos_p
!
!  CALLS: dot_product8
!  USES: MPI_ALLREDUCE
!
subroutine br_remove_prev_overlap_in_place(ivec,jvec,a)
   use bmpi_mod
   implicit none
...
!$omp    shared(br_reg) 
   do istate = ostart, ostop
      br_histbuf(istate,ivec) = br_histbuf(istate,ivec) - real(a * br_histbuf(istate,jvec), kind=lanc_prec)
   end do
!$omp end parallel do
   return
end subroutine br_remove_prev_overlap_in_place
...
!  USES: MPI_ALLREDUCE
!
subroutine br_normalize_in_place(ivec,a)
   use bmpi_mod
   implicit none
   ...
   call br_check_setup()
   dp = dot_product8(br_histbuf(ostart:ostop,ivec),br_histbuf(ostart:ostop,ivec))
   call BMPI_ALLREDUCE(dp, tdp, 1, MPI_SUM, icomm, ierr)
   a = sqrt(tdp)
   m = 1.0d0 / a
   ! KSM TODO: write loop for OMP
...
!$omp parallel do  private(i) firstprivate(m) shared(br_reg,ostart,ostop)
   do i = ostart, ostop
      br_histbuf(i,ivec) = real(REAL(br_histbuf(i,ivec), kind=8) * m, kind=lanc_prec)
   end do
!$omp end parallel do
end subroutine br_normalize_in_place
...
!$omp parallel do &
!$omp    private(istate, hi, m8, sum) &
!$omp    firstprivate(ostart, ostop, maxhi) &
!$omp    shared(br_orthogdot, br_histbuf)
      do hi = 1, maxhi
         sum = 0.0
         do istate = ostart, ostop
            sum = sum + REAL(br_reg(istate), kind=8) &
                * REAL(br_histbuf(istate,hi), kind=8)
                  end do
         br_orthogdot(hi) = sum
      end do
!$omp end parallel do
...
!   print*,' orthogf ',br_orthogdotf(maxhi)
...
!if(off==0)print*,br_orthogdotf(1:maxhi)
!$omp parallel do private(hi, sum)
   do istate = ostart, ostop ! pos on outside for locality
      sum = 0.d0
      do hi = 1, maxhi
         sum = sum + br_orthogdotf(hi) * br_histbuf(istate, hi)
      end do
      ! subtract totaled overlap
      br_reg(istate) = br_reg(istate) - REAL(sum, kind=lanc_prec)
   end do
!$omp end parallel do
...
!
! checks everything is orthogonal
!  CALLED BY: lanczos_p
!
!  CALLS:
!      dot_product8
!  USES: MPI_ALLREDUCE
!
subroutine br_check_orthogonal(loss)
...
loss = .false.
   do hi = 1,maxhi
	   if(abs(br_orthogdotf(hi)) > dot_tol)then
!		   if(iproc==0)print*,' loss of orthogonality ', maxhi+1,hi,br_orthogdotf(hi)
		   loss = .true.
...
end subroutine br_check_orthogonal
!
!  ORTHOGONALIZES in history  vec(ivec) against vectors vec(vecstart) through vec(vecstop)
!
!  CALLED BY: lanczos_p
!
!  CALLS:
...
!  USES: MPI_ALLREDUCE
!
subroutine br_orthogonalize_in_place(ivec,vecstart,vecstop)
   use bmpi_mod
   implicit none
   integer, intent(in) :: ivec,vecstart,vecstop
...
!   print*,' orthog before ',br_orthogdot(1:maxhi)
   ...
   if(vecstop-vecstart+1 > 2*ompNumThreads) then
!$omp parallel do &
!$omp    private(istate, hi, m8, sum) &
!$omp    firstprivate(ostart, ostop, maxhi) &
!$omp    shared(br_orthogdot, br_histbuf)
      do hi = vecstart,vecstop
         sum = 0.0
         do istate = ostart, ostop
            sum = sum + REAL(br_histbuf(istate,ivec), kind=8) &
                * REAL(br_histbuf(istate,hi), kind=8)
                  end do
         br_orthogdot(hi) = sum
      end do
!$omp end parallel do
      ...
      ! old version, OpenMPI inside dot_product
      do hi = vecstart,vecstop
         br_orthogdot(hi) = dot_product8(br_histbuf(ostart:ostop, hi), br_histbuf(ostart:ostop,ivec))
      end do
   end if
...
!$omp parallel do private( hi, sum)
   do istate = ostart, ostop ! pos on outside for locality
      ...
      ! subtract totaled overlap
      br_histbuf(istate,ivec) = br_histbuf(istate,ivec) - REAL(sum, kind=lanc_prec)
   end do
!$omp end parallel do
end subroutine br_orthogonalize_in_place
