...
!$omp end parallel do
if(nproc > 1) then
    call BMPI_ALLREDUCE(otmp, dalpha, 1,  MPI_SUM, icomm, ierr) 
else
     dalpha = otmp
end if
istate1 = Lstart - 1
...
!$omp end parallel do
if(nproc > 1) then
    call BMPI_ALLREDUCE(otmp, dovlp, 1, MPI_SUM, icomm, ierr) 
else
     dovlp = otmp
end if
istate1 = Lstart - 1
...
!
  if(nproc > 1)then
    call BMPI_ALLREDUCE(pover, sumover, niter,  MPI_SUM, icomm, ierr)   
  else
    do ivec=1,iter
...
!  dalpha = sumover(iter)
  ...
  if( vchar == 'n' )then
     ...
     call BMPI_ALLREDUCE(vec1, size(vec1), MPI_SUM, icomm, ierr)
  else
     ! call block_reduce(dimbasis,vec2)
     call BMPI_ALLREDUCE(vec2, size(vec2), MPI_SUM, icomm, ierr)
  end if
...
!
  if(nproc > 1)then
    call BMPI_ALLREDUCE(otmp, sumover, 1,  MPI_SUM, icomm, ierr) 
  else
       sumover=otmp
  end if
