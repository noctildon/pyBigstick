...
!
  ...
  call BMPI_BARRIER(icomm,ierr) 
  call BMPI_BCAST(spformat,3,0,icomm,ierr) 
  wcolumns =1 ! default
  ...
  case ('iso') 
     ...
     pnwtflag = .false. 
     !              backspace(1) 
  case ('wpn','WPN')  ! isospin formalism but different p,n weights in 1 column
     isoflag = .true. 
     pnwtflag = .true. 
!.... ADDED 7.9.11.......	 
  case ('pnw')  ! isospin formalism but different p,n weights in two columns
        isoflag = .true. 
        pnwtflag = .true. 
		wcolumns=2 
...
!------------------READ IN-------------------------------------------------- 
     ...
     if(wcolumns==1)then
        ...
        do i = 1, numorb(1) 
           if ( iproc == 0 ) then 
              read (1,*,end=2001) xn, xl, xj, xwp,xwn 
              orbqn(1,i)%nr = int(xn) 
              orbqn(1,i)%l = int(xl) 
              orbqn(1,i)%par = (-1)**(orbqn(1,i)%l) 
              orbqn(1,i)%j = int(2*xj) 
           ...
           call BMPI_BCAST(orbqn(1,i)%w,1,0,icomm,ierr) 
           call BMPI_BCAST(orbqn(2,i)%w,1,0,icomm,ierr) 
		   orbqn(2,i)%nr = orbqn(1,i)%nr   ! copy over
		   orbqn(2,i)%l = orbqn(1,i)%l   ! copy over
		   orbqn(2,i)%par = orbqn(1,i)%par   ! copy over
		   orbqn(2,i)%j = orbqn(1,i)%j   ! copy over
...
!------------------# OF PROTONS AND NEUTRONS------------------- 
  ...
  if ( writeout .and. iproc == 0 )then
	   write(resultfile,*)np(1),np(2) 
	   if(menu_char=='d')	   write(denresultfile,*)np(1),np(2) 
   end if
...
!  NOTE: reguires flag iffulluseph = .true. AND requires np(it) > threshhold
	 if(iffulluseph .and. np(it)==npmax(it) .and. np(it) > fulltrigger)then
		 phconj(it)=.true.
		 np(it) = 0
		 if(iproc==0)then
			 if(it==1)then
	 ...
	 if(iffulluseph .and. np(it)==npmax(it) .and. np(it) <= fulltrigger)then
		 ...
		 if(iproc==0 .and. it==1)then
			 write(6,*)' NOT invoking particle-hole conjugation for filled proton space '
			 write(6,*)' If you wish to, you must change variable fulltrigger= ',fulltrigger,' in module flagger '
			 write(logfile,*)' NOT invoking particle-hole conjugation for filled proton space '
			 write(logfile,*)' If you wish to, you must change variable fulltrigger= ',fulltrigger,' in module flagger '			 			 
		 end if
		 if(iproc==0 .and. it==2)then
			 write(6,*)' NOT invoking particle-hole conjugation for filled neutron space '
			 write(6,*)' If you wish to, you must change variable fulltrigger= ',fulltrigger,' in module flagger '
			 write(logfile,*)' NOT invoking particle-hole conjugation for filled neutron space '
			 write(logfile,*)' If you wish to, you must change variable fulltrigger= ',fulltrigger,' in module flagger '			 			 
		 end if
	 end if
...
!......... REVISED 8/2011 by CWJ TO ALLOW BOTH PARITIES........ 
  ...
  if ( writeout .and. iproc == 0 )then
	   write(resultfile,*)jz,cparity 
	   if(menu_char=='d')	   write(denresultfile,*)jz,cparity 
   end if
