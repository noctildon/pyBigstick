...
subroutine clockout_den1b
	...
	call BMPI_Reduce(pdentimearray,size(pdentimearray),MPI_SUM,0,icomm,ierr)
	
	ndentimearray = 0.d0
	
	ndentimearray(iproc)=time_n1b
	call BMPI_Reduce(ndentimearray,size(ndentimearray),MPI_SUM,0,icomm,ierr)
...
2020 continue
      ...
      call BMPI_BCAST(opwtPPNb,1,0,icomm,ierr)
      call BMPI_BCAST(opwtPNNb,1,0,icomm,ierr)
      call BMPI_BARRIER(icomm,ierr) 
...
!............ WRITE OUT SPE............
     ...
     tpoX = tottimeX/(actual_iterations*localops)*1.e9*float(num_threads_global)
	 if(localops > 0)then
...
!............ WRITE OUT PPN............
       ...
       tottimeX = 0.d0
       do ip = 0,nprocs-1
          tottimeX = tottimeX + time_procPPN(ip)*float(num_threads_global)
        end do
       tpoX = tottimeX/(actual_iterations*localops)*1.e9
       if(localops==0)then
         write(timefile,1003)'PPN',tpoPPNdefault, tpoPPNprior, tpoPPN
...
if(iproc >0)then
  tag = iproc
   ...
   call BMPI_SEND(tmp,14,0,tag,icomm,ierr) 
end if

if(iproc == 0)then
  do iprocs = 1,nprocs-1
      tag = iprocs
      call BMPI_Recv(tmp,14,iprocs,tag,icomm,stat,ierr)
...
subroutine detailed_hmult_timing  
	...
	call BMPI_Reduce(hmultarray,size(hmultarray),MPI_SUM,0,icomm,ierr)
	call BMPI_Reduce(waitarray,size(waitarray),MPI_SUM,0,icomm,ierr)
	call BMPI_Reduce(reducearray,size(reducearray),MPI_SUM,0,icomm,ierr)
