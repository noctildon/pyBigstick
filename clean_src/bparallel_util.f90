...
subroutine getMPIprocs
   ...
   if(distributeMPI)then
         ...
         if(.not.simulateMPI .and. menu_char=='m')then 
            ...
            if(nprocs < nfragments**2)then
			  ...
			  call BMPI_Barrier(icomm,ierr)
               ...
               call BMPI_Abort(icomm, 103, ierr)
            endif
            simulateMPI = .true.
         else
            nprocs = nproc
      ...
      if(nproc /= 1)then   ! A MISTAKE
		  ...
		  call BMPI_Barrier(icomm,ierr) 
          call BMPI_ABORT(icomm, 104,ierr)
      end if
      nprocs = 1
  endif
...
!............. CHECK THEY WILL FIT..............
  if (Lpiece*niter > maxpiecedim)then        ! exceeds memory
	 ...
	 call BMPI_Barrier(icomm,ierr)
     call BMPI_ABORT(icomm,203,ierr)
  end if
...
!---- COLLECT Lstart INFO FROM EACH NODE ON ROOT NODE----
  ...
  if(npiece > nprocs)then
	 ...
	 call BMPI_Barrier(icomm,ierr)
     call BMPI_ABORT(icomm,201,ierr)
...
!................... ALLOCATE.........................
  ...
  if(npiece > nprocs)then
	 ...
	 call BMPI_Barrier(icomm,ierr)
     call BMPI_ABORT(icomm,202,ierr)
