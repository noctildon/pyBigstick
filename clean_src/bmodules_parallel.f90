...
! add by Shan in 7.7.3ZZ
  real(4) :: opwtPPb, opwtPNb,opwtPPNb,opwtPNNb
!  integer(8) :: allopPPb, allopPNb,allopPPNb,allopPNNb
...
!  MOVED FROM module fragments 7.9.6   
!------------- JUMP STORAGE ACROSS FRAGMENTS (f2f matvec blocks)-------------- added in 7.6.2 -----------------
   ...
   type f2fjump
	   ...
	   real(8)    :: totopstorage   ! added in 7.9.6 for 
...
module opbundles
  ...
  type bund
	 ...
	 real :: jumpstore  ! jump storage in Gb
	 logical :: greedy! if memory is too large, assign opbundle to its own MPI proc
	 
	 real :: opwt    ! ADDED 7.9.6 
	 integer :: origin   ! ADDED 7.9.6 -- what is the original bundle? 
...
!............ ADDED IN 7.9.10 to aid jumpstart........
   type splitdat
	   character(3) :: optype
	   
	   integer :: obstart,obstop ! start, stop of opbundles
	   
	   integer :: nsplits
	   
	   integer(8) :: pxstart,pxend,nxstart,nxend
	   integer(8), allocatable :: locsplit(:)
	   real, allocatable :: opwtsplit(:)
	   
   end type splitdat
   
   type (splitdat), allocatable :: splinter(:)
...
!---- INFORMATION ON 'GREEDY' OPBUNDLES  -- ADDED 7.9.3
!   a greedy opbundle has operations that require a lot of memory, more than is available on average
!   these cause problems in distributing work and must be accounted for separately

  real(8) :: avgopstore, avgwtopstore    ! how much memory an operation (weighted operation) has available on average
  real(8) :: greedymemorytot   ! how much memory is needed by greedy opbundles
  real(8),allocatable :: greedymemoryf2f(:,:)  !  how much memory is needed by greedy opbundles on frag to frag
 															
  integer :: ngreedy  ! # OF 'GREEDY' OPBUNDLES
  logical :: use_greedy = .false. ! flag to separate out greedy opbundles into separate MPI ranks
...
!real(8), allocatable ::  time_procPPNf(:)
!real(8), allocatable ::  time_procPNNf(:)
real(8), allocatable ::  time_procPPNb(:)
real(8), allocatable ::  time_procPNNb(:)
