...
3000    continue	
		...
		call BMPI_Bcast(readjumpstart,1,0,icomm,ierr)
...
!.............. INFORMATION BEYOND THAT IN THE HEADER....
    ...
    call BMPI_BCAST(v,1,0,icomm,ierr)
!	if(iproc==0)print*,' my parity ',cparity
    ...
    call BMPI_BCAST(blockflag,1,0,icomm,ierr)
    call BMPI_BCAST(blocksize,1,0,icomm,ierr)
		
	return

1011 continue
call BMPI_Abort(icomm,321,ierr)	
...
subroutine readtimeinfo4jumpstart(nob_draft)
    ...
    call BMPI_BCAST(nob_test,1,0,icomm,ierr)
    ...
    do iob = 1,nob_draft
	    ...
	    call BMPI_BCAST(problem,1,0,icomm,ierr)
		if(problem)goto 1011
	    call BMPI_BCAST(opwt,1,0,icomm,ierr)
...
1011 continue
	call BMPI_Abort(icomm,322,ierr)		
...
!	   end if
   ...
   if(problem)then
		call BMPI_Abort(icomm,133,ierr)	
		stop
	end if
...
!			draft_opbundle(iob)%opwt = opwt
    ...
    if(problem)then
 		call BMPI_Abort(icomm,134,ierr)	
 		stop
 	end if	
	
	call BMPI_Bcast(opwtlist,size(opwtlist),0,icomm,ierr)
    do iob = 1,nob_draft
