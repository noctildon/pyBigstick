...
subroutine centroidquestion
    ...
    integer(4)   :: ierr
    character(1) :: ychar
!............. ADDED IN 7.9.7 : 3BODY CENTROIDS.......  

	  if(.not.threebody)then
		  if(iproc==0)then
			  write(6,*)'Do you want 3-body centroids (y/n) ?'
		  
			  if(auto_input)then
		          read(autoinputfile,'(a)')ychar
		       else
				   write(6,*)'(If not sure, enter "n")'
		          read(5,'(a)')ychar
		          write(autoinputfile,'(a)')ychar			  			  
			  end if
		  end if
...
end subroutine centroidquestion
!================================================================
!
...
!  deallocate(tripxxx)
  ...
  if(it == 1)then
     nmatppp = n
     if(iproc==0)print*,n,' uncoupled PPP matrix elements '
  endif
  if(it == 2)then
     nmatNNN = n
     if(iproc==0)print*,n,' uncoupled NNN matrix elements '
...
!....... DUPLICATES A PRINT OUT ELSEWHERE....   
!   if(it == 1)then
