...
!					endif
 ...
 !                      ftmp = ftmp + dv*dv
     ...
     if(useNewReorthog) then
        ...
        call BMPI_ALLREDUCE(configfrac(:), SIZE(configfrac), MPI_SUM, icomm, ierr) ! in place reduce
     end if
     if(iproc==0)then
		 write(configfile,*)' State ',istate  !,' energy ',ei
...
!............... LOOP OVER NEUTRON CONFIGURATIONS .................
	             ...
	             do in = Xconfiginfo(2)%parinfo(parn)%wstart(iwn), Xconfiginfo(2)%parinfo(parn)%wstop(iwn)
					 ipart = in+pcfindx(ip)
					 write(configfile,'(3i8,5x,f12.7)')ipart,ip,in,configfrac(ipart)
...
do iorb = 1,numorb(it)
   write(ifile,102)orbqn(it,iorb)%nr, orbqn(it,iorb)%l, orbqn(it,iorb)%j, orbqn(it,iorb)%w
enddo
write(ifile,*)' Parity = ',iparity
if(allsameW)then
    write(ifile,*)' No W truncation '
else
    write(ifile,*)' Max W-excitation = ',maxWtot -minWtot
endif
...
!    write(17,*)orbarray,Xconfiginfo(it)%nstates(iconfig), Xconfiginfo(it)%e0(iconfig)
    if(configout)then
        write(configfile,111)iconfig,(orbarray(i),i= 1,numorb(it))
111     format(i6,' : ',20i3)
    endif
end do
