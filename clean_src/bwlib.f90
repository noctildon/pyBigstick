  ...
  if ( maxWtot > 0 ) then
     ...
     call BMPI_BCAST(ychar,1,0,icomm,ierr)
	  ...
	  select case (ychar)
        ...
        call BMPI_BCAST(tempW,1,0,icomm,ierr)
        ...
        ! KSM:  FIXED MPI TYPE ERROR
        call BMPI_BCAST(tempW,1,0,icomm,ierr)
        call BMPI_BCAST(tempWp,1,0,icomm,ierr)
        call BMPI_BCAST(tempWn,1,0,icomm,ierr)
...
!----------- LOOP OVER THE S.P. STATES adding the min . max  W -------
  do nh = 1,bmin(nhsps0(it),np(it))
     minWh(it,nh) = minWh(it,nh-1)+hspsqn(it,nh)%w
     maxWh(it,nh) = maxWh(it,nh-1)+hspsqn(it,nhsps0(it)-nh+1)%w
     !        print*,nh,minWh(it,nh),maxWh(it,nh)
  enddo ! nh
...
!           is either the maximum you can fill, or subtracted off from max tot
  ...
  do nh = 0,bmin(nhsps0(it),np(it))
     
     nhc = np(it) - nh 
     if(nhc > bmin( nhsps0(-it),np(it) ) ) cycle
...
!C      print*,it,' w ceiling = ', wceiling(it)
