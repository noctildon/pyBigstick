...
! NOTE f2fjumpstat and type f2fjump MOVED to module operation_stats in 7.9.6
...
! nodal is allocated in routines setnodaltribution, setnodaltribution_alt (both here) 
! and in overlaptribution (in bdenslib1.f90)
...
! note in 7.9.11: I should probably make useNewReorthog obligatory
...
!   print*,iproc,maxfragmentsize,break_vectors,test_fragments,' testing '
...
!   use fragments
   ...
   integer(8) :: tmpNfrag,avgfragsize
   real(8)    :: xNfrag
   real(8)    :: xmaxfragmentsize

   if(maxfragmentsize/=0)goto 33
   ...
   if(ask2break_vectors)then  ! should always default to this...
	   if(nproc > 1 .or. menu_char=='m' .or. test_fragments)then
         if(iproc==0)then
		  ...
		  if(dimbasis > 1e6)then
			  
			  if(minmaxfragsize < 1e6)then
			      write(6,'("Largest un-splittable block = ",f6.3," million basis states ")')(minmaxfragsize*1e-6)
				  
			  else
			  
			      write(6,'("Largest un-splittable block = ",i5," million basis states ")')nint(minmaxfragsize*1e-6)
				  
			  end if
		  else
			  write(6,'("Largest un-splittable block = ",i5,"  basis states ")')minmaxfragsize
		  end if
			  
	!	  print*,' (Largest un-splittable block = ',minmaxfragsize,')'
		  if(minmaxfragsize < maxfragmentsize_default)then
			  
			  write(6,'(" (Default = ",i5," million basis states )" )')nint(maxfragmentsize_default*1e-6)
    !         print*,'( Default = ',maxfragmentsize_default,')'
 		  else
			  write(6,'(" (Default = ",i5," million basis states )" )')nint(minmaxfragsize*1e-6)
...
!... PUT IN HINT IF Z /= N.....
		  ...
		  if(dimbasis < 1e6 )then
               print*,'( Enter fragment size in # of states;  enter 0 to use default )'
		   else
               print*,'( Enter fragment size in millions of states (NEW); enter 0 to use default )'
			   if(blockstrengthflag)then
				   print*,'( Be sure that fragmentsize x dim block is not larger than 2 billion )'
			   end if
			   
			   
		   end if
		   if(auto_input)then
			   read(autoinputfile,*)xmaxfragmentsize
		   else
		   
              read(5,*,err=1111)xmaxfragmentsize
			  write(autoinputfile,*)xmaxfragmentsize
    	   end if
		   if(dimbasis .ge. 1e6)then
			     if(xmaxfragmentsize > 1e7)then  ! allow for mistakes
				     maxfragmentsize = nint(xmaxfragmentsize)
			     else
			        maxfragmentsize = nint(xmaxfragmentsize*1e6)
			     end if
		   else
			      maxfragmentsize = nint(xmaxfragmentsize)
		   end if
...
!..... ADDED IN 7.9.1.... ADJUSTS TO GET BEST FIT.....
           if(maxfragmentsize < dimbasis*0.8)then
			    xNfrag = real(dimbasis,8)/real(maxfragmentsize,8)
			    tmpNfrag = int(xNfrag)
			    if(xNfrag > tmpNfrag+0.2)then
				   tmpNfrag = tmpNfrag +1
			    end if
			    if(tmpNfrag > 1)then
				  maxfragmentsize = dimbasis/tmpNfrag
				  print*,' Auto balancing fragment size to ',maxfragmentsize
...
!          print*,' '
		 !  write(autoinputfile,'(i12,"    !  FRAGMENT SIZE (0 = use default)")')nint(maxfragmentsize*1e-6)
         end if
...
!..... ADDED 7.6.5......NEED TO LEAVE ROOM FOR FRAGMENTSIZE IN AUTOINPUT
         if(auto_input)then
			 ...
			 write(autoinputfile,'("0     !  LANCZOS FRAGMENT SIZE in millions (0 = use default)")')
		 end if
...
!     fragmenter (above)
!
! NOTE: Modified in 7.9.1: instead of trying to get each fragment close to the maxfragmentsize,
! try to get sum fragmentsize = maxfragmentsize
!
...
!         if(localbasiscount + nnsd*localpsd > maxfragmentsize .and. i < nphblocks)then  ! reset
         if(localbasiscount + nnsd*localpsd > localfragsize .and. i < nphblocks)then  ! reset
            ...
            if(i==1) then   ! close out fragment at prior sector 
               if(localbasiscount==0)then
                  ...
                  print*,localbasiscount,nnsd*localpsd,localfragsize,maxfragmentsize
                  print*,isector,nfrag
                  stop
  				   end if
			...
			localfragsize = maxfragmentsize*nfrag - sumbasis
!			print*,localbasiscount,' trying to fit ',localfragsize
   ...
   if(iproc==0 .and. .not. create .and. (nproc > 1 .or. menu_char=='m'))then
      print*,' There are a total of ',nfrag,' Lanczos fragments'
      print*,' from a new total of ',nss,' sectors (old = ', nsectors(1),')'
!	  print*,'  NEED TO RECREATE SECTORS DATA!!! '
      print*,' min/max fragment dimension = ',minfragsize,maxfragsize
   end if
   maxfragsize_all=maxfragsize
   if(udbg /= 0) then ! KSM
...
! ADDED 7.4.9: cannot subtract nodes if only one node assigned
   ...
   if (iproc == 0 .and. (nproc > 1 .or. menu_char=='m')) print *, "Intermediate unused MPI procs", nodediff,' out of ',nnodesx
