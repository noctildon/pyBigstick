...
101           continue
		...
		call BMPI_BCAST(use3body,1,0,icomm,ierr)
...
!... NOW BROADCAST
		...
		call BMPI_Bcast(mymaxorb,1,0,icomm,ierr)
		call BMPI_Bcast(numorbmax,1,0,icomm,ierr)
        ...
        call BMPI_BCAST(tmpvec,size(tmpvec),0,icomm,ierr)
        if(iproc/=0)then
		   i = 0
		   do a = 1,mymaxorb
			  do b = 1,a
