...
!  integer :: ncheck_ex        ! # of extra states to check for convergence  ! MOVED TO MODULE convergence
...
!  integer :: thick_restart_factor , thick_restart_min   ! MOVED TO MODULE convergence
  integer :: nthick_add_def = 5   ! default # of extra states to add for thick-restart; must be >= ncheck_ex
!  integer :: nthick_add   ! # of extra states to add for thick-restart; must be >= ncheck_ex ! MOVED TO MODULE convergence
...
!   real    :: maxjumpmemory_default  = 64.0    ! in Gb  KSM - for DM runs on Edison
   
   real :: maxfrac_sectjump_store  = 0.99  ! what fraction of maxjumpmemory_default can jumps in a single sector jump take up
...
!   logical :: annex_bundles = .false. ! if TRUE, then seek to annex bordering bundles which share jumps ADDED 7.8.0
   ...
   integer(4) :: fulltrigger = 12  ! only use iffulluseph if number of particles > fulltrigger; 
   ...
   logical,parameter :: disableNoOMPloopswitch  = .true.  ! if T, disables switching order of loops when there is no OpenMP
                              ! NOTE do not set .false., will cause problems in some cases, for reasons not understood
   ...
   integer,parameter :: pstridecut = 1000     ! (typically ~1000) in applying NN, if there is no OpenMP, 
	...
	logical :: confirm_orthogonality = .false. ! ADDED 7.9.4, checks orthogonalizty 
